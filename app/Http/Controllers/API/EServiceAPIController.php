...
class EServiceAPIController extends Controller
{
    ...
    public function trackClick(Request $request){
        $track=new TrackClick;
        $track->user_id=$request->input('user_id');
        $track->service_id=$request->input('service_id');
        $track->type=$request->input('type');
        $track->save();
        return $this->sendResponse(array('track_id'=> $track->id),'Tracking recorded successfully');
        
    }
    public function trackBooked(Request $request){
        $id=$request->id;
        $track=TrackClick::find($id);
        $track->isBooked=true;
        $track->update();
        return $this->sendResponse($track,'Tracking recorded successfully');
